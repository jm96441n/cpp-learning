...
void rotate_all(vector<Shape*>& v, int angle) // rotate all members of v by angle
{
  for(auto p : v)
    p->rotate(angle);
}
...
Circle* build_circle_from_input(istream& in)
{
  int clat;
  int clng;
  int rad;
  ...
  in >> rad;
  cout << endl;
  return new Circle(Point(clat, clng), rad);
}

Triangle* build_triangle_from_input(istream& in)
{
  int clat;
  int clng;
  int height;
  ...
  in >> height;
  cout << endl;
  return new Triangle(Point(clat, clng), height);
}

Smiley* build_smiley_from_input(istream& in)
{
  int clat;
  int clng;
  int rad;
  ...
  in >> rad;
  cout << endl;
  Smiley* s = new Smiley{Point(clat, clng), rad};
